...
def compute_steps_for_sliding_window(image_size: tuple[int, ...], tile_size: tuple[int, ...], tile_step_size: float) -> list[list[int]]:
    ...
    for dim in range(len(tile_size)):
        ...
        actual_step_size = max_step_value / (num_steps[dim] - 1) if num_steps[dim] > 1 else 99999999999
...
if __name__ == "__main__":
    ...
    assert all(i == j for i, j in zip(a_padded.shape, (4, 2, 48, 27), strict=False))
    assert all(i == j for i, j in zip(a_npy_padded.shape, (4, 2, 48, 27), strict=False))
